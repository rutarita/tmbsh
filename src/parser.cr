  ...
  class Parser
    ...
    private def unexpected_token(*expected)
      raise UnexpectedToken.new(
        "Unexpected token: #{token.kind} #{token.raw_value} at #{row}:#{column} #{expected.size > 0 ? ::String.build { |io| io << "Expected: "; expected.each { |e| io << " '#{e}'" } } : ""}"
      )
    end
    ...
    def parse_lambda : Interpreter::LambdaNode
      next_token
      argnames, optional_start_index, last_is_splat = token.kind.parenthesis_open? ? parse_args_definition : NO_ARGUMENTS
      skip_whitespaces
